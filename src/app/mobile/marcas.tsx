      ...
      <Marquee speed={50} gradient={false} pauseOnHover>
        {operadoras.map((operadora, index) => (
          <button
            ...
            className="mx-8 flex items-center justify-center focus:outline-none cursor-pointer"
          >
            <img
              src={operadora.logo}
              alt={operadora.nome}
              className="h-[60px] sm:h-[50px] w-auto transition-transform duration-300 hover:scale-110"
            />
          </button>
        ))}
      </Marquee>
